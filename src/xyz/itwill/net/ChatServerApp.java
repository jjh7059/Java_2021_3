...
//채팅 서버 프로그램 - 다중 스레드
// => 클라이언트가 보내온 메시지를 전달받아 접속된 모든 클라이언트에게 전달하는 기능
...
public class ChatServerApp {
	...
	public ChatServerApp() {
		...
		try {
			...
			System.out.println("[메시지]채팅 서버 동작중...");
	...
	public class SocketThread extends Thread {
		...
		//클라이언트가 보내온 메시지를 전달받기 위한 입력 스트림을 저장하기 위한 필드
		private BufferedReader in;
		
		//클라이언트가 메시지를 전달하기 위한 출력 스트림을 저장하기 위한 필드
		...
		// => 클라이언트의 메시지를 전달받아 접속 클라이언트에게 전달하는 기능 제공
		@Override
		public void run() {
			...
			try {
				...
				//out = new PrintWriter(socket.getOutputStream());//출력 버퍼를 이용 (flush 작성해야함)
				//PrintWriter(OutputStream out, boolean autoFlush)
				// => autoFlush 매개변수에 true를 전달하면 출력 버퍼 사용하지 않고 전달하는 기능 제공
				out = new PrintWriter(socket.getOutputStream(), true);
				
				//클라이언트에서 보내온 대화명을 반환받아 저장
				aliasName = in.readLine();
				
				//현재 접속된 모든 클라이언트에게 입장 메시지를 전달
				...
				// => 콜렉션 필드에서 현재 접속되었던 클라이언트와 연결된 소켓정보 제거
				// => 현재 접속중인 모든 클라이언트에게 퇴장 메시지를 전달
