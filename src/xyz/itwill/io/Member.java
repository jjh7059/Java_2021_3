...
// => 객체 직렬화(Object Serialization) : 인스턴스를 byte 배열(원시데이터 모임)로 변환하여
//입력 또는 출력하는 기능을 제공

//객체 직렬화 클래스로 선언하는 방법
//1. java.io.Serializable 인터페이스를 상속받아 선언 (보편적으로 사용)
// => 인스턴스의 모든 필드값을 byte 배열로 변환하여 입력 또는 출력하는 기능 제공
//2. java.io.Externalizable 인터페이스를 상속받아 선언 (필드가 많을 때 사용)
// => 인터페이스의 추상메소드를 오버라이드 선언하여 입력 또는 출력에 대한 변환 처리
