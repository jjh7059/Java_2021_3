...
//Byte Stream : 원시데이터로 전달하기 위한 입력 또는 출력 스트림
// => InputStream 클래스 또는 OutputStream 클래스를 상속받은 자식 클래스로 생성된 스트림(인스턴스)
...
public class ByteStreamApp {
	public static void main(String[] args) throws IOException {
		...
		while(true) {
			...
			//InputStream.read() : 입력 스트림에 존재하는 값을 원시 데이터(1byte)로 읽어 반환하는 메소드
			// => 입력 스트림에 값이 없는 경우 스레드 일시 중지
			// => 키보드를 눌러 입력 스트림에 값을 전달하고 엔터(코드값 - 13)를 눌러 스레드를 재실행
			...
			// => OuputStream 클래스를 상속받은 자식클래스(PrintStream)로 생성된 스트림
			//OutputStream.write(int b) : 원신 데이터(1Byte)를 출력 스트림으로 전달하는 메소드
