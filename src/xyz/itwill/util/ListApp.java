...
// => 요소의 저장순서가 존재하며 첨자를 이용하여 요소 처리
// => 동일한 값의 요소 중복 저장 가능
public class ListApp {
	public static void main(String[] args) {
		List<String> list = new ArrayList<String>();
		
		//List.add(E element) : List 인스턴스에 요소(Element)를 삽입하는 메소드
		list.add("홍길동");
		list.add("임꺽정");
		list.add("전우치");
		list.add("일지매");
		...
		//List.add(int index, E element) : List 인스턴스의 원하는 위치에 요소(Element)를 삽입하는 메소드
		list.add(4, "장길산");
		System.out.println("list = " + list);
		System.out.println("======================================");
		
		//List.remove(E element) : List 인스턴스에 저장된 요소를 제거하는 메소드
		// => 요소(인스턴스)의 값을 비교한 후 제거
