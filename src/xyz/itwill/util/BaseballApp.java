...
// => 난수와 입력값은 3개의 정수값으로 서로 달라야하고 0으로 시작 불가능
// => 입력값이 형식에 맞지 않을 경우 재입력(입력횟수 미증가)
// => 난수와 입력값을 비교하여 동일한 정수값이 존재할 때 위치가 같으면 스트라이크, 다르면 볼로 계산
// => 스트라이크가 3개면 성공 메시지 출력 후 프로그램 종료하고 아니면 스트라이크와 볼의 갯수 출력
...
public class BaseballApp {
	public static void main(String[] args) {
		...
		for(int cnt = 1; cnt <= 15; cnt++) {
			...
			for(int i = 0; i < dap.length; i++) {//난수값에 대한 배열 요소 첨자
				for(int j = 0; j < num.length; j++) {//입력값에 대한 배열 요소 첨자
			...
			if(strike == 3) {
				System.out.println("[메시지]축하합니다. " + cnt + "번만에 맞췄습니다.");
				result = true;
				break;
			}
		...
		if(!result) {//정답을 맞추지 못한 경우
			...
			System.out.println("[메시지]정답을 못 맞추셨습니다. 정답은 [" + temp + "]입니다.");
		}
		in.close();
	}
}
