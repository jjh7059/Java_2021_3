...
public class SwingApp extends JFrame {
	...
	public SwingApp(String title) {
		...
		// => [Alt] + [문자]로 메뉴를 선택 : 라벨명에 문자가 존재할 경우 _ 표시
		menu.setMnemonic('F');
		...
		//KeyStroke.getKeyStroke(int keyCode, int modifiers) : 기능키와 문자가 저장된 KeySroke 인스턴스를
		//생성하여 반환하는 메소드
		...
		//JFrame.getContainer() : 프레임의 Container 인스턴스를 반환하는 메소드
		...
		// => WindowConstraints.DO_NOTHING_ON_CLOSE : 아무런 동작도 발생되지 않도록 설정
		// => WindowConstraints.HIDE_ON_CLOSE : 프레임을 숨기도록 설정(기본)
		// => WindowConstraints.DISPOSE_ON_CLOSE : 메모리 정리 후 프로그램이 종료되도록 설정
		// => WindowConstraints.EXIT_ON_CLOSE : 무조건 프로그램이 종료되도록 설정
